...
class DeepgramTranscriber(BaseTranscriber):
  ...
  def __transcribe_single_chunk(self, audio_chunk, language=None):
    logging.info(f"{LOG_TAG_DEEPGRAM} Starting transcription of single chunk...")
    transcript = self.__transcribe_audio_file(audio_chunk, language)
    logging.info(f"{LOG_TAG_DEEPGRAM} Finished transcribing single chunk.")
    return transcript

  def __split_audio(self, audio):
    logging.info(f"{LOG_TAG_DEEPGRAM} Splitting audio into smaller chunks...")
    audio_chunks = [audio[i:i + self.MAX_CHUNK_LENGTH_IN_MS] for i in range(0, len(audio), self.MAX_CHUNK_LENGTH_IN_MS)]
    logging.info(f"{LOG_TAG_DEEPGRAM} Audio split into {len(audio_chunks)} chunks.")
    return audio_chunks

  def __generate_chunk_files(self, audio_chunks):
    logging.info(f"{LOG_TAG_DEEPGRAM} Generating temporary chunk files...")
    chunk_file_paths = []
    ...
    logging.info(f"{LOG_TAG_DEEPGRAM} Created {len(chunk_file_paths)} chunk files from the original audio.")
    return chunk_file_paths

  def __delete_chunks(self):
    logging.info(f"{LOG_TAG_DEEPGRAM} Deleting temporary chunk files...")
    shutil.rmtree(self.OUTPUT_CHUNKS_FOLDER_PATH)
    os.makedirs(self.OUTPUT_CHUNKS_FOLDER_PATH)
    logging.info(f"{LOG_TAG_DEEPGRAM} Temporary chunk files deleted.")
  ...
  def __threading_transcription(self, chunk_file_paths, language=None) -> str:
    ...
    logging.info(f"{LOG_TAG_DEEPGRAM} Processing {len(chunk_file_paths)} chunks with {max_workers} concurrent workers...")
  ...
  def transcribe(self, audio_file_path, language=None) -> str:
    ...
    try:
      logging.info(f"{LOG_TAG_DEEPGRAM} Loading audio file: {audio_file_path}")
      if language:
        logging.info(f"{LOG_TAG_DEEPGRAM} Using specified language: {language}")
      else:
        logging.info(f"{LOG_TAG_DEEPGRAM} Auto-detecting language...")
      ...
      logging.error(f"{LOG_TAG_DEEPGRAM} An error occurred during transcription: {e}")
